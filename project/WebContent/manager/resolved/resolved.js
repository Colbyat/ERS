...
window.onload = () => {
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState === 4 && xhr.status === 200) {
			...
			for(let i = 0; i<reimbursements.length; i++) {
				...
				let resolvingManager = document.createElement("td")
				resolvingManager.innerText = reimbursements[i].resolvingManager
				row.append(resolvingManager)
	...
	xhr.open('GET', '../servlets/allresolved')
	xhr.send()
